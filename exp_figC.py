...
ambiguity = int(sys.argv[5]) # <--------------------------- ARG 5 -------------------------

sampling = int(sys.argv[6]) # <--------------------------- ARG 6 -------------------------

#sampling = 1 #1: sampling; 0:deterministic
path = "results/figC_sampling/"
num_repetitions = 5

if sampling != 1:
    path = "results/figC_determ/"
    num_repetitions = 1


filename = 'res_exp_figC_m_' + str(num_model) + '_d_' + str(num_dataset) + '_css_' + str(cs_size) + \
           '_k_' + str(gen_mt) + '_s_' + str(ambiguity) + '.csv'
...
## RUN
rseed = ((((5 + num_model)*7 + num_dataset)*11 + cs_size)*13 + kDB_learn)*17+ambiguity
np.random.seed(rseed)
random.seed(rseed)

gen_model = orig_models[num_model]
gen_cm = orig_conf_mtrcs[num_model]
