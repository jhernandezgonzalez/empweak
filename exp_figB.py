...
full_set_size = int(sys.argv[4]) # <--------------------------- ARG 4 -------------------------
scene = int(sys.argv[5]) # <--------------------------- ARG 5 -------------------------

sampling = int(sys.argv[6]) # <--------------------------- ARG 6 -------------------------
#1: sampling; 0:deterministic

path = "results/figB_sampling/"
num_repetitions = 5

if sampling != 1:
    path = "results/figB_determ/"
    num_repetitions = 1


filename = 'res_exp_figB_m_' + str(num_model) + '_d_' + str(num_dataset) + '_fss_' + str(full_set_size) + \
...
gen_mt_name = "figB_simplegen.pkl"
if gen_mt > 1:
    gen_mt_name = "figB_complexgen.pkl"
...
num_orig_datasets = int(len(orig_datasets) / num_orig_models / 10)

## RUN
rseed = ((((5 + num_model)*7 + num_dataset)*11 + full_set_size)*13 + kDB_learn)*17+scene
...
datasets = orig_datasets[ (num_model*num_orig_datasets+num_dataset)*10+full_set_size]
